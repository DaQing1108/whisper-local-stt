...
### 本機 API（127.0.0.1:5001）沒有身份驗證層

- **現況**：所有 route（包含 `/config` 寫入憑證、`/api/system-audio/start` 觸發音訊擷取）都沒有 token/密碼驗證，CORS 只限制瀏覽器 JS 呼叫者。
- **為什麼這是刻意的**：server 只綁 `127.0.0.1`，這是本機 companion app 常見的信任邊界（同類工具如 Ollama、LM Studio 也是同樣模式），依賴 OS 層級的行程隔離，不是應用層驗證。攻擊者要能打到這支 server，前提是已經能在使用者的 Mac 上執行任意程式碼，那個情境下這層驗證已經不是重點。
- **不要做**：不要因為看到「無驗證」就自動加上 token/密碼機制——這會增加 Obsidian plugin 端 token 交換/輪替的複雜度，換來的實際安全效益極低，不成比例。
- **確認時間**：2026-07-10（Whisper app 完整架構 review 後，使用者確認維持現況）

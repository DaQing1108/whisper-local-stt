## 2026-07-15｜SwiftUI＋Python AI Core 深度評估、實際使用差異與決策建議
...
### Appendix D：對 Alex 實際使用 Whisper App 的差異與優缺點

#### BLUF

如果只批准 Phase 0＋Phase 1，Alex 日常使用的 Whisper App 幾乎不會有明顯差異。這兩個階段主要是架構整理與技術驗證；現有 v2.3.1 仍是正式日用版本，SwiftUI PoC 只驗證「選音檔轉錄」，暫時不取代麥克風、系統音訊或混音功能。

真正有感的差異，要到後續完成 microphone、ScreenCaptureKit、Settings、history 與 release migration 才會出現。

#### 各階段的實際影響

| 階段 | Alex 會看到什麼 | 日常使用影響 |
|---|---|---|
| Phase 0 | Python Core 內部整理 | 原則上完全無感 |
| Phase 1 | 獨立 SwiftUI 測試 App，只能選音檔轉錄 | 可試用，但不取代正式版 |
| Phase 2 | 原生麥克風錄音 | 開始有明顯差異 |
| Phase 3 | 系統音訊、混音、Settings、history 等功能回歸 | 才可能正式取代舊版 |
| Phase 4 | 簽章、notarization、Sparkle 與 release hardening | 安裝與更新體驗改善 |

#### 可能有感的優點

1. **App 啟動與關閉更乾淨**：未來可減少 port 5001、殭屍 server、視窗關閉後背景程序仍存活等問題，Worker 異常時也能顯示錯誤並重啟。
2. **麥克風及系統音訊權限較穩定**：麥克風不再依賴 WKWebView `getUserMedia` runtime patch；ScreenCaptureKit 可由主 App 管理相同 TCC identity。這項優勢要到 Phase 2／3 才會實現。
3. **取消轉錄更可靠**：按下取消後可真正停止模型運算、清理 temporary files，並開始下一份工作，不必整個關閉 App。
4. **更符合 macOS 操作習慣**：原生 Settings、Menu、keyboard shortcut、file picker、notifications、Dock 與 Dark Mode 行為可更一致。
5. **安裝及更新可能更可靠**：若 Phase 4 成功，可使用 Developer ID、notarization 與標準 Sparkle integration，降低首次安裝與更新後失效風險。

#### 不會自然改善的部分

1. **辨識準確度不會因 SwiftUI 自動提升**：mlx-whisper、faster-whisper、OpenAI Whisper、pyannote、OpenCC 與 LLM post-processing 仍由同一套 Python AI Core 執行。
2. **轉錄速度不一定明顯變快**：主要耗時仍是 model loading、Whisper inference、LLM 與 diarization；UI framework 不是主要瓶頸。
3. **App 體積不一定大幅縮小**：現有約 401 MB 的主要來源是 Python runtime、native ML dependencies、ffmpeg 與 frameworks，不只是 Web UI。

#### 實際缺點與風險

1. **初期會有正式版與 PoC 兩個版本**：必須使用不同名稱、圖示與 bundle identifier，避免誤用及設定互相污染。
2. **第一個 SwiftUI 版本功能會少很多**：PoC 不含 microphone、system audio、mixed audio、Notion／Obsidian、diarization、完整 Settings 與 Sparkle。
3. **可能發生 feature regression**：現有 JavaScript 包含 chunking、WakeLock、recording recovery、history、詞庫、batch transcription、SSE reconnect 等大量隱性行為，不能只重做畫面。
4. **Swift＋Python 會增加雙棧維護成本**：未來部分功能需要同時修改 Swift UI、JSONL protocol 與 Python Worker；若 ownership 與 contract tests 不清楚，架構可能比現在更難維護。

#### 對目前使用情境的切換條件

Alex 目前依賴 microphone、Teams／Zoom system audio、mixed mode、長時間會議、Notion／Obsidian、LLM post-processing 與 Speaker Diarization，因此不應在 Phase 1 結束後切換正式版。

較安全的正式切換條件：

1. 麥克風 standard／live mode 通過長時間實測。
2. ScreenCaptureKit 系統音訊穩定。
3. 至少保留現有 Notion／Obsidian workflow。
4. cancellation、crash recovery 與 settings migration 通過。
5. 正式簽章 App 可在沒有開發環境的 Mac 執行。
6. 相同測試音檔的辨識品質不低於現版。
7. 現版保留一段 rollback 期間。

#### 使用者價值結論

Phase 0＋1 對 Alex 現在幾乎無感，但它以較低成本驗證未來能否得到一個啟動、權限、取消、更新及 crash recovery 更可靠的 Whisper App，而不犧牲目前已可用的 AI 功能。

如果 PoC 無法證明可靠性明顯改善，就不應繼續遷移；維持 v2.3.1 並針對既有架構局部修正，反而更符合實際使用利益。

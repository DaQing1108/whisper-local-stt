...
Phase: 混音錄音停止後畫面沒同步最終結果（codex-handoff/codex-receive）
Working: 使用者實測一場近 3 小時混音會議（entry `0CC894E9`，實際 2.5h / 9092s，segments coverage 100%，資料完整無遺失），按「停止」後畫面沒顯示逐字稿結果，得自己去 History 才找到。追蹤確認根因：`ContentView+CaptureActions.swift` 的 `stopMixedAudioRecording()` 在停止當下會直接呼叫一次 `history.updateResult(...)` 把累積逐字稿寫回 history，但這次呼叫完全繞過畫面更新（沒呼叫 `restore()`）——對照組是錄音進行中每個 chunk 完成時走的 `acceptCompletedChunk` 分支，每次都會 `restore(entry)` 正常同步畫面，只有停止那次額外寫入漏了這步；且最後一段音訊還在非同步轉錄時，畫面上完全沒有「還在收尾」的提示。用 `/codex-handoff` 產出 `HANDOFF_CODEX_MIXED_AUDIO_STOP_UI_SYNC.md`（AC-1~6，人工列出），透過 Agent 交另一個 Claude 開發：新增 `CaptureUIRules.shouldSyncFinalizedMixedAudioResult(isDraftDirty:)` 純函式（`!isDraftDirty`，跟既有 diarization/retranscribe 攔截同慣例——history 寫入無條件執行、只有「要不要動畫面」受 guard 控制）；`stopMixedAudioRecording()` 捕捉 `updateResult` 回傳值，guard 通過才 `restore(updated)`；`transcriptContent` 加「⏳ 混音錄音已停止，最後片段仍在轉錄中…」提示，直接讀既有的 `mixedAudioRecording.isDraining`（未新增判斷邏輯）。對方本機 commit（`29b8e8a`+`ad9e5f0`）不 push。`/codex-receive` 獨立驗證：`swift build` 乾淨、新增測試（`finalizedMixedAudioResultSyncsOnlyWhenDraftIsClean`）通過、完整 `swift test` 196 tests 僅 5 issues（SPIKE 測試需 env var + `LiveRecordingControllerTests` 一個 device-recovery 並行負載 flake，跟上次任務同一組已知模式，非本次引入）、diff 範圍精確（4 檔）、SPIKE 未動、schema 未改、無 iCloud 衝突複本。FF-merge 進 `whisper-swift`，push（pre-push hook Python 307 tests 全過）→ `ad9e5f0`。
Next action：真機走查 AC-2（停止時草稿乾淨 vs 有手動編輯兩種情境）+ AC-3（收尾提示外觀與消失時機），連同上一輪的 TranscriptOverwriteGuard 兩個確認對話框，一起併進下次 build/安裝走查。混音模式停止時「session 合併音檔」與「最後一段轉錄」兩件事的時序（audioPath 有時指向已被清掉的 chunk 檔而非 session 檔）仍待另開任務查根因。
Blockers: Gate E（Developer ID notarization / 乾淨 Mac 測試 / Sparkle）仍待使用者提供 Apple Developer 憑證，尚未開始；`MixedAudioRecordingController.swift` 及其測試仍是既有未收尾 WIP（SPIKE：`WHISPER_DEBUG_SEPARATE_TRACKS`），連續兩次任務皆刻意未觸碰

## Checkpoint History
### 2026-09-05｜混音錄音停止後畫面同步最終結果（codex-handoff/codex-receive）
- Completed: 追蹤使用者實測近 3 小時混音會議「停止後看不到結果、要去 History 找」的根因——`stopMixedAudioRecording()` 停止當下的 `history.updateResult` 寫入繞過畫面更新（沒呼叫 `restore()`），且無「收尾中」提示。透過 codex-handoff/codex-receive 跨帳號分工修復：新增 `CaptureUIRules.shouldSyncFinalizedMixedAudioResult`、`stopMixedAudioRecording()` 補 `restore(updated)`（受 draft-dirty guard，history 寫入本身不受影響）、`transcriptContent` 加收尾提示（讀既有 `isDraining`）
- State: `swift build` 乾淨；新增測試通過；完整 `swift test` 196 tests，5 issues 皆為已知非本次引入（SPIKE env var + LiveRecordingController 並行 flake）；diff 範圍精確（4 檔），未碰 MixedAudio SPIKE / schema；FF-merge `whisper-swift`，push（pre-push hook 307 Python tests 全過）→ `ad9e5f0`
- Next: 真機走查（併同上輪 TranscriptOverwriteGuard 對話框一起）；混音 session 合併音檔與最後一段轉錄的時序問題另開任務

### 2026-09-04 12:45｜逐字稿破壞性覆寫攔截 + 備份（codex-handoff/codex-receive）+ 被覆寫會議重轉恢復
- Completed:
  (1) 診斷使用者早上一筆 55 分鐘混音會議（entry `F8CF06E9`）逐字稿被「辨識講者／重新轉錄」截斷成 ~90 秒的根因：混音模式 `segments` 掉尾（整個 history 12 筆低 coverage 全是 mixed-audio）+ `ContentView+Results.swift` 兩條路徑（diarization `.onChange` 整段重建、retranscribe `updateResult` 無條件覆寫）無縮水攔截、無寫入前備份。原文無備份可還原。
  (2) 恢復資料：`mlx_whisper` large-v3 對原始 WAV 重轉，1627 段 / ~50k 字完整逐字稿已交付（桌面 `會議逐字稿恢復_2026-09-04_53D99520/`）。
  (3) 修復（codex-handoff → 另一個 Claude 開發 → codex-receive）：新增 `TranscriptOverwriteGuard.isDestructiveShrink`（純函式，縮水 < 50% 且原文 ≥ 200 字才攔截）；`TranscriptionHistoryStore` `updateText`/`updateResult` 覆寫前寫 pre-change JSON 快照到 `history-backups/<entryID>/`（每 entry 保留 5 份、best-effort）+ `latestBackup(for:)`；兩條路徑接上確認對話框（新 `PendingRetranscribeResultOverwrite`）。
- State: `swift build` 乾淨無 warning；新增 20 測試 20/20（獨立重跑）；`LiveRecordingControllerTests` 單獨 19/19（完整套件失敗為並行負載 flake，每次失敗測試不同、單獨全過）；清掉 3 個 iCloud 衝突複本 `* 2.swift`（破壞編譯、非本次程式碼）；diff 範圍精確、SPIKE 未動、schema 未改。FF-merge `whisper-swift`，push（pre-push hook Python 307 tests 全過）→ `43f9eab`（`58765e7` 實作 + `43f9eab` 驗證報告，開發方本機 commit、Claude Code 驗收後 push）。
- Next: 真機走查兩個確認對話框（併進下次 build/安裝）；混音 `segments` 掉尾根因另開任務；AC-8 還原選單 UI 依約略過留 follow-up。

### 2026-08-24 21:00｜講者改名 CJK IME 修復 + 歷史記錄「重新轉錄」功能（codex-handoff/codex-receive）
- Completed: 修復講者重新命名 sheet 的中文輸入法組字被打斷問題（`Binding(get:set:)` 缺乏穩定身份 → 改用原生 array binding）；連同 3 週前的講者改名 WIP 一起收尾 commit（`90e5711`，刻意排除 Mixed Audio 那批無關 WIP）；用 spec-writer 鎖定 AC 後，透過 codex-handoff/codex-receive 跨帳號分工（peer session `whisper-48`）完成「歷史記錄重新轉錄」功能，修復歷史記錄缺 segments 導致辨識講者按鈕永久灰掉的問題，順手補上 `WorkerSupervisor.transcribe()` 缺少 `diarizationOperationInProgress` guard 的既有 gap
- State: 講者改名修復已真機驗證中文輸入正常；`swift test` 全量 182 tests，僅 1 個既有無關失敗（Mixed Audio spike 測試）；`swift build` 兩次改動皆乾淨；commit `90e5711`、`2e27708` 皆已 push `origin/whisper-swift`；歷史記錄重新轉錄功能尚未真機驗證（待打包後測試）
- Next: 打包安裝＋真機驗證「重新轉錄」按鈕實際跑通


### 2026-08-03 12:05｜版號 bump 0.2.3 + Notion checkpoint 頁面修復
- Completed: 講者辨識任務真機驗證完整通過後，依累積修復批次 bump 版號慣例，將 `CFBundleShortVersionString` 從 `0.2.2` bump 到 `0.2.3`（commit `e4cea54`），涵蓋講者辨識修復、裝置事件去重、混音睡眠/喚醒修復、Capture UI/UX 重構；重新 `build_swiftui_app.sh` 並安裝到 `~/Applications/Whisper Swift.app`，`defaults read` 確認版號生效。另外修復先前寫入 Notion「工作總結倉庫」時漏帶 `parent` 參數導致誤建成獨立頁面的問題，用 `notion-move-pages` 移入正確 data source 並補回屬性
- State: `defaults read ~/Applications/Whisper\ Swift.app/Contents/Info.plist CFBundleShortVersionString` 回傳 `0.2.3`；Notion page `3b1280a95f7681979dbefb811332f7f0` 的 `<ancestor-path>` 已正確指向工作總結倉庫 data source，Tags/Status/BLUF/Content/Date 屬性皆已補回
- Next: 無，此任務結案

### 2026-08-03 11:04｜講者辨識過度分裂修復 + 講者重新命名功能
- Completed: 診斷出 `Speaker 31/33/39/46/55` 等異常高編號的根因是 `FastClusteringConfig()` 用預設參數對真實會議過度分裂；透過 codex-handoff（產出 `HANDOFF_CODEX_SPEAKER_DIARIZATION_FIX.md`，10 條 AC）交給另一個 Claude Code 帳號實作，對方 commit 但不 push；codex-receive 獨立驗收：逐檔比對 `git show` diff 確認無範圍外洩、本機重跑全部驗收指令結果一致、push 前 pre-push hook 額外驗證全專案 307 個 Python 測試
- State: pytest 25/25（本次新增）+ 307/307（全專案，push hook 驗證）全過；`SpeakerRenameTests` 5/5、完整 `swift test` 175/175 全過；`git diff --stat` 僅 7 個既有檔案改動 + 2 個新檔案（`SpeakerRename.swift`/`SpeakerRenameTests.swift`），`TranscriptionHistoryEntry`/`TranscriptionSegment`/`package.sh`/`FastClusteringConfig` 預設值皆未觸碰；commit `4cece05` 已 push `origin/whisper-swift`
- Next: 使用者下次打包/跑真實會議錄音時，實際驗證「已知講者人數」欄位是否確實減少過度分裂、「重新命名講者」sheet 的實際操作體驗

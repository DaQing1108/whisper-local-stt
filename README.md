...
## Current State
Last checkpoint: 2026-09-04 12:45
Phase: 逐字稿破壞性覆寫 — 攔截 + 備份（codex-handoff/codex-receive）+ 一筆被覆寫會議的重轉恢復
Working: 使用者早上一筆 55 分鐘混音會議（entry `F8CF06E9`）原本 Obsidian note 完整，跑「辨識講者／重新轉錄」後 history 與 note 都只剩開頭 ~90 秒（10 段 / 273 字）。調查確認：(1) 這是混音模式系統性 bug——整個 history 84 筆中 coverage < 80% 的 12 筆全是 mixed-audio，`segments` 陣列會掉尾；(2) `ContentView+Results.swift` 的 diarization `.onChange` 用 `renderSpeakerLabeled(segments)` 整段重建 `transcriptDraft`、retranscribe 完成路由用 `updateResult` 無條件覆寫，兩條路徑都沒有「新結果比現有短很多」的攔截、也沒有寫入前備份。原文無法從任何備份還原（無 TM local snapshot、無當期 history backup、Obsidian trash 空）。**恢復**：音檔完整，直接用 `mlx_whisper` large-v3 對原始 WAV 重轉，得回 1627 段 / ~50k 字的完整逐字稿，已交付使用者（桌面 `會議逐字稿恢復_2026-09-04_53D99520/`）。**修復**：用 `/codex-handoff` 產出 `HANDOFF_CODEX_TRANSCRIPT_OVERWRITE_GUARD.md`（AC-1~8，人工列出非 spec-writer），透過 Agent 交另一個 Claude 開發：新增純函式 `TranscriptOverwriteGuard.isDestructiveShrink`（縮水 < 50% 且原文 ≥ 200 字才攔截）、`TranscriptionHistoryStore` 在 `updateText`/`updateResult` 覆寫前寫 pre-change JSON 快照到 `history-backups/<entryID>/`（每 entry 輪替保留 5 份、best-effort 失敗不阻塞主寫入）、新增 `latestBackup(for:)`、兩條破壞性路徑接上確認對話框（新 `PendingRetranscribeResultOverwrite`）。對方本機 commit（`58765e7`+`43f9eab`）不 push。`/codex-receive` 獨立驗證：清掉 3 個 iCloud 衝突複本（`* 2.swift`，非本次程式碼、破壞編譯）後 `swift build` 乾淨無 warning、新增 20 測試 20/20、`LiveRecordingControllerTests` 單獨 19/19（完整套件的失敗是並行負載 flake，每次失敗的測試不同、單獨跑全過）、diff 範圍精確、SPIKE 未動、schema 未改。FF-merge 進 `whisper-swift`，push（pre-push hook Python 307 tests 全過）→ `43f9eab`。AC-8（還原選單 UI）依約略過留 follow-up，`latestBackup(for:)` 函式＋測試已做。
Next action：真機走查兩個確認對話框（辨識講者縮水、重新轉錄縮水 → 取消不動、確認才覆寫、`history-backups/` 有舊版），建議併進下次 build/安裝（反正要過 Gatekeeper）。混音 `segments` 掉尾的根因（Python worker / MixedAudio 管線）仍待另開任務。
Blockers: Gate E（Developer ID notarization / 乾淨 Mac 測試 / Sparkle）仍待使用者提供 Apple Developer 憑證，尚未開始；`MixedAudioRecordingController.swift` 及其測試仍是既有未收尾 WIP（SPIKE：`WHISPER_DEBUG_SEPARATE_TRACKS`），本次刻意未觸碰

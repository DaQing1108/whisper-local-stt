...
真機驗證（使用者）：C ✅ D ✅ 已在真實混音路徑確認——無 domain/key 錄一段（Minerva→密尼瓦/秘密碑亂跳、無標點）對比有設 domain/key 錄一段（密涅瓦每次一致、專利/認知差/政務委員機要秘書/長春藤盟校/暢銷書全正確、有句讀），證實 `build_prompt`→`initial_prompt` 與 `llm_punctuate` 皆生效。
Next action：A（音量平衡）與 B（切點對齊）仍缺「舊 build 同素材」對照才能定論改善幅度——併同前四輪積壓的真機走查一次做完（TranscriptOverwriteGuard 兩對話框、混音停止畫面同步、silence-detector 分窗化 coverage、振幅腰斬修復、本輪 A/B），一次 build 後全部測，通過後再一起 bump 0.2.4→0.2.5。B 的 `findEnergyValley` 非重疊窗口（獨立 review 標 MEDIUM，已加註解）、正規化常數 3000/100 為合成向量經驗值，真機不佳時微調 `PCM16Mixer.normalizationTargetRMS`/`normalizationMinimumRMS`。`mixed-mode-stopgap` 分支已 push 留痕。混音停止 session 合併音檔時序問題仍待另開任務。
...
- Next: A（音量平衡）B（切點）缺舊 build 同素材對照才能定論；併同前四輪積壓真機走查一次做完，通過後一起 bump 0.2.4→0.2.5。`findEnergyValley` 非重疊窗口（review MEDIUM，已註解）、正規化常數 3000/100 真機不佳時微調 `normalizationTargetRMS`/`normalizationMinimumRMS`
...
- Next: 真機錄一段有停頓的測試音檔驗證 coverage 改善；混音振幅減半（既有 SPIKE）是否為複合成因待確認；混音停止時 session 音檔時序問題另開任務

...
## Current State
Last checkpoint: 2026-08-24 21:00
Phase: 講者改名 CJK IME 修復 + 歷史記錄「重新轉錄」功能 — 已完成、已 push，待打包真機驗證
Working: 使用者回報「辨識講者，重新命名講者，如果輸入中文名稱，會產生錯誤」——診斷後發現根因是 `SpeakerRenameSheetView.swift` 每個 TextField 用手動建構的 `Binding(get:set:)`（存進 `[String:String]` 字典），沒有跨 render 的穩定身份，導致中文輸入法（拼音/注音）打字時的「組字中」marked text 狀態，在任何無關的重新渲染時被打斷或清空——英文輸入不受影響因為沒有組字中間狀態。改用跟 `labels` 位置對應的 `[String]` 陣列＋原生 `$inputs[index]` array binding（SwiftUI 對 `Binding<Array>` 的 `RandomAccessCollection`/`MutableCollection` 原生 subscript，身份穩定）修復。連同同一 worktree 裡三週前就留下、已測試但未提交的講者改名相關 WIP（`SpeakerRenameSheetData` 取代 Bool+dict 的 sheet 呈現方式、`PendingDiarizationOverwrite` 覆蓋確認對話框）一起 commit（`90e5711`），刻意排除另一批同期但有 13 個測試失敗、未收尾的 `MixedAudioRecordingController.swift` WIP。真機驗證：使用者用拼音/注音輸入中文講者名稱，確認組字正常、逐字稿正確套用，不再亂碼。打包安裝到 `~/Applications/Whisper Swift.app` 過程中意外發現 `cp -R` 產生了一個 `Contents 2` 衝突資料夾（`codesign --verify` 報 unsealed contents），清除後簽名驗證恢復正常；也確認到覆蓋磁碟檔案不會讓正在跑的舊程序換血，需要使用者 Cmd+Q 舊程序後重開才吃到新 code。接著使用者提出延伸問題「歷史會議記錄無法做 AI summary 及辨識講者」——AI Summary 經使用者重測確認正常（非 bug）；辨識講者的根因確認是 `TranscriptionHistoryStore.swift` 的 `segments` 欄位是後期才加入 schema，早期歷史記錄 decode 時 fallback 成 `[]`，且「辨識講者」按鈕的啟用條件就是 `!segments.isEmpty`，永久卡死無回填路徑。依使用者要求，用 `/spec-writer` 鎖定 10 條 AC 產出規格（`docs/Whisper_History_Retranscribe_Spec_v1.md`），再用 `/codex-handoff` 產出 `HANDOFF_CODEX_HISTORY_RETRANSCRIBE.md`，透過 SendMessage 交給另一個 Claude Code 帳號（peer session `whisper-48`）實作「重新轉錄」功能：對 segments 為空且原始音檔仍存在的歷史記錄提供「重新轉錄」按鈕，用 entry 當時的 model/language/domain/extraTerms 重新呼叫轉錄，完成後透過既有 `TranscriptionHistoryStore.updateResult(id:)` 寫回同一筆記錄（不建立新記錄、不覆寫 obsidianNotePath/notionChildPageID、不強制切換畫面），並修正一個順手發現的既有 gap（`WorkerSupervisor.transcribe()` 先前沒有檢查 `diarizationOperationInProgress`）。對方本機 commit（`2e27708`）後產出驗收報告，`/codex-receive` 獨立驗證：逐行核對 diff 內容（非只信任摘要）、本機重跑 `WorkerSupervisorTests` 15/15、`TranscriptionHistoryStoreTests` 10/10、全量 `swift test` 182 tests 僅 1 個既有無關的 `MixedAudioRecordingControllerTests` spike 測試失敗（3 issues，需要特定環境變數才會真正跑到斷言，與本次改動無關）、`swift build` 乾淨，皆與對方回報一致，`git diff --stat` 確認 6 個檔案改動全在允許範圍內、未誤觸 Mixed Audio WIP。commit `2e27708` 已 push `origin/whisper-swift`。過程中對方發現 HANDOFF/規格文件裡「既有失敗數固定 13 個」這個數字跟他們實測環境不符（base commit 本身測試結果不穩定，2/3 次全綠、1 次 flaky），已記錄在驗證報告第 3.3 節，供未來規格撰寫時核對，不影響本次功能正確性判斷。
Next action：重新打包（`scripts/build_swiftui_app.sh`）安裝到 `~/Applications/Whisper Swift.app`，使用者手動 Cmd+Q 舊程序＋重開＋Gatekeeper 核准，真機驗證「重新轉錄」按鈕實際跑通（對一筆缺 segments 的歷史記錄按下後，完成時辨識講者按鈕變為可點擊）
Blockers: Gate E（Developer ID notarization / 乾淨 Mac 測試 / Sparkle）仍待使用者提供 Apple Developer 憑證，尚未開始；`MixedAudioRecordingController.swift` 及其測試仍是 3 週前留下、有 13 個測試失敗、未收尾的獨立 WIP，本次兩個任務皆刻意未觸碰

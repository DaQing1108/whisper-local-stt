...
struct SystemAudioCaptureLifecycleTests {
    ...
    @Test
    func recordingControllerWritesBackendPCMToFinalizedWAV() async throws {
        let permission = SystemAudioPermissionController(provider: LifecyclePermissionProvider(granted: true))
        permission.refresh()
        let backend = FakeSystemAudioCaptureBackend()
        ...
        let controller = SystemAudioRecordingController(
            lifecycle: lifecycle, backend: backend, transcriber: FakeSystemAudioTranscriber()
        )
        let url = FileManager.default.temporaryDirectory.appendingPathComponent("system-recording-\(UUID()).wav")
        defer { try? FileManager.default.removeItem(at: url) }

        try await controller.start(outputURL: url)
        backend.emitPCM(Data([0x01, 0x02, 0x03, 0x04]))
    ...
    @Test
    func finalizedSystemAudioWAVIsHandedToWorker() async throws {
        let permission = SystemAudioPermissionController(provider: LifecyclePermissionProvider(granted: true))
        permission.refresh()
        let backend = FakeSystemAudioCaptureBackend()
        let lifecycle = SystemAudioCaptureLifecycleController(permission: permission, backend: backend)
        let transcriber = FakeSystemAudioTranscriber()
        let controller = SystemAudioRecordingController(
            lifecycle: lifecycle,
            backend: backend,
            transcriber: transcriber
        )
        let url = FileManager.default.temporaryDirectory.appendingPathComponent("system-worker-\(UUID()).wav")
        defer { try? FileManager.default.removeItem(at: url) }
        ...
        #expect(finalized == controller.sessionFinalizedURL)
        #expect(finalized != url)
        #expect(transcriber.receivedURL == url)
    }

    @Test
    func runtimeCaptureErrorCannotBeHiddenBySuccessfulStop() async throws {
        let permission = SystemAudioPermissionController(provider: LifecyclePermissionProvider(granted: true))
        permission.refresh()
        let backend = FakeSystemAudioCaptureBackend()
        let lifecycle = SystemAudioCaptureLifecycleController(permission: permission, backend: backend)
        ...
        let url = FileManager.default.temporaryDirectory.appendingPathComponent("system-runtime-error-\(UUID()).wav")
        defer { try? FileManager.default.removeItem(at: url) }

        try await controller.start(outputURL: url)
        backend.emitPCM(Data([0x01, 0x02]))
        lifecycle.handleRuntimeError(SystemAudioCaptureError.conversionFailed("bad sample"))

        #expect(!controller.canStart)
        #expect(controller.canStop)

        await #expect(throws: SystemAudioRecordingError.self) {
